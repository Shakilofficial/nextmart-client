...
const ProductCard = ({ product, className, index = 0 }: ProductCardProps) => {
  ...
  const stockPercentage = product?.stock
    ? Math.min(100, (product.stock / 20) * 100)
    : 0;
    ...
    >
      <Card
        className={cn(
          ...
          isHovered
            ? "shadow-lg scale-[1.02] z-10 border-gray-300"
          ...
          <div className="absolute top-2 left-2 flex flex-col gap-1">
            {product?.stock === 0 ? (
              ...
              <motion.div
                ...
                transition={{ duration: 0.3 }}
              >
                <Badge className="px-2 py-1 text-xs font-medium bg-rose-500 text-white rounded-md">
            ...
            {lowStock && (
              ...
              >
                <Badge
                  variant="outline"
                  className="px-2 py-1 text-xs font-medium bg-amber-50 text-amber-700 border-amber-200 rounded-md"
                >
          ...
          >
            ...
            <Link href={`/products/${product?._id}`}>
              <Button
                variant="secondary"
                size="sm"
                className="rounded-full bg-white/90 backdrop-blur-sm shadow-sm hover:bg-rose-500 hover:text-white transition-colors h-9 w-9"
              >
